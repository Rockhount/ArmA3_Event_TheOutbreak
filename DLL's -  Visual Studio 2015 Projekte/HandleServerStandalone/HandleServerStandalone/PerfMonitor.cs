            ...
            while (true)
            {
                ...
                lock (LockThis)
                {
                    if (PlayersStats.Count > 0)
                    {
                        ...
                        var SumVal4 = (TotalReadedDataByteCount > 0) ? Math.Round((double)TotalReadedDataByteCount / 1048576, 2, MidpointRounding.AwayFromZero) : 0;
                        var SumVal5 = (TotalWrittenDataByteCount > 0) ? Math.Round((double)TotalWrittenDataByteCount / 1048576, 2, MidpointRounding.AwayFromZero) : 0;
                        PerfLog.Add(GetFormatedConsoleText(RefText, string.Format("Summarized|{0}({1}%)|{2}({3}%)|{4}MB|{5}MB", SumVal0, SumVal1, SumVal2, SumVal3, SumVal4, SumVal5)));
